(() => {
  ...
  const renderUniverseCard = (universe) => `
    <a class="universe-card reveal" data-universe="${universe.id}" href="${universe.url}"${externalAttributes(universe)} aria-label="${universe.cta} — ${universe.name}">
      ...
      <div class="universe-card__top"><span>${universe.number} / ${universe.category}</span><span>Universo</span></div>
  ...
  document.querySelector("#universeGrid").innerHTML = config.universes.map(renderUniverseCard).join("");
  document.querySelector("#footerUniverses").innerHTML = config.universes.map(renderFooterLink).join("");
  ...
  const organizationData = {
    ...
    url: config.groupUrl,
    email: config.email,
    description: "Casa creativa mexicana que reúne moda, joyería y diseño de interiores.",
    sameAs: [config.social.instagram, config.social.pinterest],
    subOrganization: config.universes.map(({ name, url }) => ({ "@type": "Organization", name, url }))
  };
